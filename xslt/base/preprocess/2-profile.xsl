...
<xsl:template match="/">
  ...
  <xsl:sequence select="f:profile(/,
                                  $profile.separator, $profile.lang,
...
<xsl:function name="f:profile" as="document-node()">
  ...
  <xsl:apply-templates select="$root" mode="mp:profile">
    ...
    <xsl:with-param name="profile.outputformat" select="$profile.outputformat"/>
    ...
    <xsl:with-param name="profile.wordsize" select="$profile.wordsize"/>
  </xsl:apply-templates>
</xsl:function>
...
<xsl:template match="*" mode="mp:profile">
  ...
  <xsl:if test="fp:profile-ok(@lang, $profile.lang, $profile.separator)
                ...
                and fp:profile-ok(@arch, $profile.arch, $profile.separator)
                ...
                and fp:profile-ok(@outputformat, $profile.outputformat, $profile.separator)
                ...
                and fp:profile-ok(@wordsize, $profile.wordsize, $profile.separator)
                ">
    <xsl:copy>
      <xsl:copy-of select="@*"/>
      <xsl:apply-templates mode="mp:profile">
