...
<xsl:template name="t:callout-bug-X">
  <xsl:param name="conum" select='1'/>
  ...
  <xsl:choose>
    <xsl:when test="$iconum &lt;= number($callout.unicode.number.limit)">
      <xsl:choose>
...
<xsl:template name="t:verbatim-patch">
...
<xsl:template match="db:areaspec|db:areaset">
  <xsl:apply-templates select="db:area"/>
</xsl:template>
...
<xsl:template name="tp:format-as-verbatim">
  ...
  <xsl:call-template name="t:verbatim-patch">
